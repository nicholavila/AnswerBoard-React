...
const EditQuestion = () => {
    ...
    useEffect(() => {
        ...
        ; (async () => {
            let { id } = params;
            let { data } = await Http.get(`/admin/questions/${id}`)

            if (data.success) {
                let yearIdx = data.years.findIndex(year => year._id === data.question.year);

                for (let i in data.modules) {
                    if (data.modules[i].subject === data.question.subject)
                        data.modules[i].shown = true;

                    for (let j in data.question.modules) {
                        if (data.modules[i]._id === data.question.modules[j])
                            data.modules[i].checked = true;
                    }
                }

                for (let i in data.topics) {
                    for (let j in data.question.modules) {
                        if (data.topics[i].module === data.question.modules[j])
                            data.topics[i].shown = true;
                    }

                    for (let j in data.question.topics) {
                        if (data.topics[i]._id === data.question.topics[j])
                            data.topics[i].checked = true;
                    }
                }

                for (let i in data.subtopics) {
                    for (let j in data.question.topics) {
                        if (data.subtopics[i].topic === data.question.topics[j])
                            data.subtopics[i].shown = true;
                    }

                    for (let j in data.question.subtopics) {
                        if (data.subtopics[i]._id === data.question.subtopics[j])
                            data.subtopics[i].checked = true;
                    }
                }

                setYears(data.years)
                setSubjects(data.years.length ? data.years[yearIdx].subjects : [])
    ...
    const onUpdate = async (question, { resetForm }) => {
        let { id } = params
        ...
        let { data } = await Http.put(`admin/questions/${id}`, question)
        if (data.success) {
                                ...
                                <Col md={6}>
                                    ...
                                    <Form.Group className='mb-3'>
                                        ...
                                        <Editor
                                            ...
                                            name='description'
                                            initialValue={values.markingCrit}
                                        />
                                        <Form.Control.Feedback type='invalid'>
                                            {errors.markingCrit}
                                        </Form.Control.Feedback>
                                    ...
                                    <Form.Group className='mb-3'>
                                        ...
                                        <Editor
                                            ...
                                            name='description'
                                            initialValue={values.sampleSolution}
                                        />
                                        <Form.Control.Feedback type='invalid'>
